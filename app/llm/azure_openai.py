...
Respondé UNICAMENTE con la palabra CATALOGO o la palabra PREGUNTA."""
...
def get_intent_completion(question: str) -> str:
    ...
    si el mensaje es un pedido de catalogo o una pregunta puntual. Separada
    de get_chat_completion porque usa un system prompt distinto y no necesita
    contexto del RAG."""
    response = _client.chat.completions.create(
        model=settings.azure_openai_chat_deployment,
        messages=[
